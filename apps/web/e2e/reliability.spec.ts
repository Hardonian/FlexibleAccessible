...
test.describe("reliability surfaces", () => {
  test("system page lists core service rows from live health", async ({
    page,
  }) => {
    await signInAsDemo(page);
    await page.goto("/system");
    await expect(
      page.getByRole("heading", { name: /system & core services/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: /first-run/i }),
    ).toBeVisible();
    ...
    await expect(
      list.locator('[data-service-id="worker-runtime"]'),
    ).toBeVisible();
  });

  test("dashboard renders org overview without error when platform is healthy", async ({
    page,
  }) => {
    await signInAsDemo(page);
  ...
  test("findings page distinguishes empty org data from hard failure when DB is up", async ({
    page,
  }) => {
    await signInAsDemo(page);
