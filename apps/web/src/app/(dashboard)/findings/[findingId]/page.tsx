...
import { 
  ArrowLeft, ExternalLink, AlertTriangle, ShieldAlert, AlertCircle, CheckCircle2, 
  Layers, Clock, ShieldCheck, Zap, Activity, BookOpen, Bug, Network, Shield,
  Info, ArrowRight, History
} from 'lucide-react';
...
export default async function FindingDetailPage({
  ...
  const impactConfig = {
    ...
    SERIOUS: { icon: AlertTriangle, color: 'text-orange-700', bg: 'bg-orange-50', border: 'border-orange-200' },
    MODERATE: { icon: AlertCircle, color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200' },
    MINOR: { icon: Info, color: 'text-sky-700', bg: 'bg-sky-50', border: 'border-sky-200' },
  }[finding.impact] || { icon: Bug, color: 'text-slate-700', bg: 'bg-slate-50', border: 'border-slate-200' };
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex items-center gap-2 text-sm text-slate-500 font-medium tracking-wide mb-2 transition-colors">
        <Link href="/findings" className="flex items-center gap-1 hover:text-brand-600 focus-visible:outline-brand-600 rounded px-1 -mx-1">
          <ArrowLeft className="h-4 w-4" />
          Findings
        </Link>
        <span className="text-slate-300">/</span>
        <span className="text-slate-900 truncate max-w-[200px] sm:max-w-xs">{finding.fingerprint.slice(0, 8)}...</span>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div className="flex-1 w-full space-y-6 min-w-0">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm relative overflow-hidden group">
            ...
            <div className={`absolute top-0 left-0 w-1.5 h-full ${impactConfig.bg} group-hover:bg-opacity-80 transition-colors`} />
            
            <div className="p-6 sm:p-8 pl-8 sm:pl-10">
              <div className="flex items-start gap-4">
                <div className={`shrink-0 p-3 rounded-xl ${impactConfig.bg} ${impactConfig.border} border`}>
                  <ImpactIcon className={`h-6 w-6 ${impactConfig.color}`} />
                </div>
                <div className="min-w-0">
                  <div className="flex items-center flex-wrap gap-2 mb-2">
                    <span className={`badge border ${impactConfig.bg} ${impactConfig.color} ${impactConfig.border} px-2.5 py-1 text-xs font-semibold uppercase tracking-wider`}>
                      {finding.impact}
                    </span>
            ...
            <dl className="grid grid-cols-2 md:grid-cols-4 gap-4 px-6 sm:px-8 py-5 bg-slate-50 border-t border-slate-100/60">
              <div>
                <dt className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1 px-1 flex items-center gap-1.5"><Network className="h-3.5 w-3.5"/> Site</dt>
                <dd className="font-medium text-slate-900 bg-white border border-slate-200 rounded-md px-3 py-1.5 shadow-sm truncate" title={finding.site.domain}>
                  {finding.site.name}
                </dd>
              </div>
              <div>
                <dt className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1 px-1 flex items-center gap-1.5"><Layers className="h-3.5 w-3.5"/> Occurrences</dt>
              ...
              <div className="col-span-2">
                <dt className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1 px-1 flex items-center gap-1.5"><Shield className="h-3.5 w-3.5"/> WCAG Scope</dt>
                <dd className="text-sm font-medium text-slate-700 flex flex-wrap gap-1.5">
                  {finding.wcagTags.length > 0 ? finding.wcagTags.map(tag => (
                    <span key={tag} className="bg-white border border-slate-200 rounded text-xs px-2 py-1 shadow-sm">{tag}</span>
                  )) : <span className="text-slate-500 italic px-2 py-1">Unspecified tagging</span>}
                </dd>
              </div>
            </dl>
          </div>
          ...
          {finding.cluster && (
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 relative overflow-hidden">
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <h2 className="text-lg font-bold text-slate-900">Component Topology</h2>
              </div>
              <p className="text-sm text-slate-600 mb-4 max-w-2xl">This finding maps to a recognized UI component cluster. Fixing it here will likely resolve it across multiple pages.</p>
              
              <Link href={`/clusters/${finding.cluster.id}`} className="group inline-flex items-center gap-4 p-4 rounded-lg bg-slate-50 border border-slate-200 hover:border-brand-300 hover:shadow-sm transition-all w-full">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-base font-semibold text-slate-900 group-hover:text-brand-600 truncate">{finding.cluster.name}</h3>
                  <p className="text-sm text-slate-500 truncate">{finding.cluster.description}</p>
                </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="p-5 sm:p-6 border-b border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-bold text-slate-900">Intelligence & Synthesis</h2>
                </div>
                <span className="badge bg-amber-50 text-amber-700 border border-amber-200">AI Assisted</span>
              </div>
              <div className="divide-y divide-slate-100">
                {finding.suggestions.map((suggestion) => (
                  <div key={suggestion.id} className="p-5 sm:p-6 hover:bg-slate-50/50 transition-colors">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                      ...
                      <Link href={`/remediation/${suggestion.id}`} className="btn-secondary text-xs sm:opacity-0 sm:group-hover:opacity-100 transition-opacity whitespace-nowrap">
                        Review Fix →
                      </Link>
                    </div>
                    <p className="text-sm text-slate-700 font-medium mb-4 leading-relaxed max-w-3xl">{suggestion.rationale}</p>
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div className="flex flex-col rounded-lg border border-slate-200 bg-slate-50 overflow-hidden">
                        <div className="px-3 py-2 border-b border-slate-200 bg-white text-xs font-semibold tracking-wide text-slate-500 uppercase">Original Source</div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="p-5 sm:p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <h2 className="text-lg font-bold text-slate-900">Observed Occurrences</h2>
                <p className="text-sm text-slate-500 mt-1">Showing {finding.occurrences.length} instances across {finding.site.name}. Limited to 50 for display.</p>
              </div>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-slate-50/80 border-b border-slate-200">
                    <tr>
                      <th className="px-5 py-3.5 font-semibold text-slate-600">Page Context</th>
                      <th className="px-5 py-3.5 font-semibold text-slate-600">DOM Target</th>
                      <th className="px-5 py-3.5 font-semibold text-slate-600">Failure Summary</th>
                      <th className="px-5 py-3.5 font-semibold text-slate-600 text-right">Age</th>
                    </tr>
                  </thead>
                    ...
                    {finding.occurrences.map((occ) => (
                      <tr key={occ.id} className="hover:bg-slate-50/50 transition-colors group">
                        <td className="px-5 py-4 w-1/4 max-w-[200px]">
                          <p className="font-medium text-slate-900 truncate" title={occ.page.title ?? occ.page.url}>
                            {occ.page.title ?? (occ.page.url.length > 30 ? occ.page.url.slice(0,30)+'...' : occ.page.url)}
                          </p>
                          <a href={occ.page.url} target="_blank" rel="noopener noreferrer" 
                        ...
                        <td className="px-5 py-4 w-1/4 max-w-[200px]">
                          <code className="text-[11px] font-mono bg-slate-100 text-slate-700 px-2 py-1 rounded truncate block border border-slate-200" title={occ.selector}>
                            {occ.selector}
                          </code>
                        </td>
                        <td className="px-5 py-4 w-2/4">
                          {occ.lastRawViolation?.elementContext ? (
                            ...
                            <span className="text-xs text-slate-400 italic">No context recorded</span>
                          )}
                        </td>
                        <td className="px-5 py-4 text-right text-xs text-slate-500 tabular-nums">
                          {occ.lastSeenAt.toLocaleDateString(undefined, { month: 'short', day: 'numeric'})}
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div className="w-full lg:w-96 shrink-0 space-y-6 lg:sticky lg:top-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 sm:p-6">
            ...
            <div className="mt-5 pt-4 border-t border-slate-100">
              <p className="text-xs font-medium text-slate-500 mb-2 uppercase tracking-widest">Platform Note</p>
              <p className="text-xs text-slate-600 leading-relaxed">
                Modifying the status locks it from auto-reopening unless a new verification scan overrides it. Closing an issue without a code fix will likely result in reopening upon the next structural crawl.
              </p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden text-sm">
            ...
            <div className="p-4 space-y-3">
              <div className="flex flex-col gap-1">
                <span className="text-xs font-medium text-slate-500">Source Truth</span>
                <span className="font-medium text-slate-900">
                  {finding.evidenceSource === 'AUTOMATED_AXE' ? 'axe-core Automated Engine' : 
                   finding.evidenceSource === 'MANUAL_REVIEW' ? 'Operator Verified' : 'External Import'}
                </span>
              </div>
              
              <div className="flex flex-col gap-1">
                <span className="text-xs font-medium text-slate-500">Last Telemetry Run</span>
                <span className="text-slate-700 truncate">
                  {finding.lastScanRun ? (
                    <span className="flex items-center gap-1.5" title={finding.lastScanRun.id}>
                      <Activity className="h-3 w-3 text-slate-400" />
                      {finding.lastScanRun.status.charAt(0) + finding.lastScanRun.status.slice(1).toLowerCase()}
                      {finding.lastScanRun.completedAt && ` • ${finding.lastScanRun.completedAt.toLocaleDateString()}`}
                    </span>
              ...
              {finding.evidenceSource === 'AUTOMATED_AXE' && (
                <div className="pt-2">
                  <span className="text-xs font-medium text-slate-500 block mb-1.5">Freshness Status</span>
                  {automationFreshness === 'current' ? (
                    <div className="flex p-2.5 bg-emerald-50 border border-emerald-100 rounded-lg text-emerald-800 text-xs">
                      ...
                      <span>Data is highly synced. Last verified: {finding.lastVerifiedAt?.toLocaleDateString()}.</span>
                    </div>
                     ...
                     <div className="flex p-2.5 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-xs shadow-sm">
                        ...
                        <span><strong>Stale Ledger:</strong> A newer scan has completed since this was last verified ({finding.lastVerifiedAt?.toLocaleDateString()}).</span>
                     </div>
                  ) : (
                 ...
                 <div className="space-y-6">
                   {finding.statusEvents.map((ev, i) => (
                     <div key={ev.id} className="relative pl-6">
                       {/* Node Marker */}
                       <div className={`absolute top-1 left-[-5px] w-[9px] h-[9px] rounded-full border-2 border-white ring-2 ring-transparent bg-slate-300
                       ...
                       <div className="flex flex-col gap-1 text-sm">
                         <div className="flex items-center gap-2 flex-wrap text-xs">
                           ...
                           <span className="text-slate-500 tabular-nums font-mono text-[10px]">
                             {ev.createdAt.toLocaleDateString(undefined, {month:'short', day:'numeric'})}
                           </span>
                         </div>
                         {ev.user ? (
                           <div className="text-xs text-slate-500">
                             via <span className="font-medium">{ev.user.name ?? ev.user.email}</span>
                           </div>
                         ) : (
                         ...
                         {ev.note && (
                           <div className="mt-1.5 p-2.5 bg-slate-50 border border-slate-100 rounded-md text-xs text-slate-700 italic relative break-words">
                             ...
                             <span className="relative z-10 whitespace-pre-wrap">{ev.note}</span>
                           </div>
                         )}
                       </div>
                     </div>
