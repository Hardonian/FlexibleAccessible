...
function noticeFromSearchParams(searchParams: Awaited<PageProps['searchParams']>) {
...
export default async function BillingPage({ searchParams }: PageProps) {
  ...
  const billingResult = await runOrgScopedQuery(orgRes, async (organizationId) => {
    ...
    <div className="space-y-8 max-w-6xl">
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-slate-600">
          Premium access is enforced server-side. Upgrade here to unlock the private dashboard,
          exports, automation, operator tools, and saved organization history.
        </p>
      </div>
      ...
      {!stripeReady && (
        <RouteReliabilityNotice variant="warning" title="Stripe is not configured">
          <p>
            This deployment can show billing status, but checkout and portal handoff stay disabled
            until Stripe environment variables are configured on the server.
          </p>
        </RouteReliabilityNotice>
      )}
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1.3fr)_minmax(22rem,1fr)]">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h2 className="text-lg font-semibold text-slate-950">Current subscription</h2>
              <p className="mt-1 text-sm text-slate-500">
                Status, limits, and plan-level entitlement for this organization.
              </p>
            </div>
            {canManageBilling && (
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <Stat
              label="Status"
              value={(subscription?.status ?? 'ACTIVE').toLowerCase().replace('_', ' ')}
            />
            <Stat label="Domains" value={String(subscription?.maxDomains ?? 1)} />
            <Stat
              label="AI access"
        ...
        <div className="rounded-3xl border border-slate-200 bg-slate-950 p-6 text-slate-50 shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-300">
            Public scan stays free. Every private org surface is protected at the route,
            action, and API level.
          </p>
          <ul className="mt-5 space-y-3 text-sm text-slate-300">
            <li>Free: marketing pages and public scan results.</li>
            <li>Paid: dashboard, findings, reports, exports, reviews, remediation, and automations.</li>
            <li>Billing managers can recover or upgrade even when premium access is currently blocked.</li>
          </ul>
        </div>
      </section>
      ...
      <section className="space-y-4">
        <div>
          <h2 className="text-2xl font-semibold text-slate-950">Choose a plan</h2>
          <p className="mt-1 text-sm text-slate-500">
            Prices are monthly. Upgrade flow opens Stripe Checkout and entitlement sync happens via webhook.
          </p>
        </div>
                ...
                <div className="mt-6">
                  {plan.isPaid ? (
                    <form action={startSubscriptionCheckoutAction}>
                      ...
                      <button
                        type="submit"
                        className={plan.tier === 'PROFESSIONAL' ? 'btn-primary w-full min-h-[44px]' : 'btn-secondary w-full min-h-[44px]'}
