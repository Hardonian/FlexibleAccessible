...
export default async function SettingsPage() {
  ...
  if (orgRes.kind === 'platform_blocked') {
    return (
      <div className="space-y-6 max-w-4xl">
        ...
        <RouteReliabilityNotice variant="error" title="Settings require a working database" showSystemLink={canViewSystem}>
          <p>Organization settings cannot be loaded until core data services are healthy.</p>
  ...
  if (!orgResult.ok || !orgResult.data?.organization) {
    return (
      <div className="space-y-6 max-w-4xl">
        <h1 className="text-2xl font-bold text-slate-900">Settings</h1>
        <RouteReliabilityNotice variant="error" title="Could not load organization" showSystemLink={canViewSystem}>
          <p>{orgResult.ok ? 'Organization not found or access was revoked.' : orgResult.message}</p>
        </RouteReliabilityNotice>
      </div>
    );
  }
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500">Status: {subscription.status.toLowerCase()}</p>
              </div>
            ...
            <tbody>
              {org.memberships.map((m) => (
                <tr key={m.id} className="border-b border-slate-100">
                  <td className="py-2 font-medium text-slate-900">{m.user.name ?? 'Unnamed'}</td>
          ...
          <ul className="space-y-2" role="list">
            {org.integrationConnections.map((conn) => (
              <li key={conn.id} className="flex items-center justify-between py-2 border-b border-slate-100">
                <div>
                  <span className="badge bg-slate-100 text-slate-700 mr-2">{conn.type.toLowerCase()}</span>
