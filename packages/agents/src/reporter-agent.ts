...
type AggregateData = {
  ...
  clusters: Pick<
    IssueCluster,
    "name" | "severity" | "pageCount" | "findingCount"
  >[];
  scanRuns: Pick<
    ScanRun,
    | "id"
...
export class ReporterAgent extends BaseAgent {
  ...
  async execute(context: AgentContext): Promise<AgentResult> {
    ...
    try {
      ...
      const data = await this.runStep(
        "aggregate",
        async (): Promise<AggregateData> => {
      ...
      const metrics = await this.runStep(
        "compute_metrics",
        async (): Promise<ReportMetrics> => {
      ...
      const report = await this.runStep("generate_report", async () => {
        ...
        return { reportId: created.id, summary: created.summary };
      });
